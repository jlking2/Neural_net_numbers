...
def NN_2layer_log(alpha, L, M, Xdata_train, ydata_train): 
...
def NN_2layer_Relu(alpha, L, M, Xdata_train, ydata_train): 
    p = np.shape(Xdata_train)[1] #features
    n = np.shape(Xdata_train)[0] #examples
    ...
    for epoch in range(L):
        ...
        for i in ind:
            # Forward-propagate
            testo = np.hstack((np.ones((1,1)), Xb[[i],:]@W))
            print(testo)
            H = Relu(np.hstack((np.ones((1,1)), Xb[[i],:]@W)))
            #print(np.shape(H))
            Yhat = Relu(H@V)
            # Backpropagate
            delta = (Yhat-Y[[i],:])*Yhat*(1-Yhat)
    ...
    ## Final predicted labels (on training data)
    H = Relu(np.hstack((np.ones((n,1)), Xb@W)))
    Yhat = Relu(H@V)
...
def Get_error_NN_log(alpha,L,M,x_train_0D,y_train):
    ...
    for rr in range(0,10):
        for k in range(0,y_length):
            if y_train[k] == rr:
                ...
                y_eval[k] = 0
        train_output = NN_2layer_log(alpha, L, M, x_train_0D, y_eval)
...
#Default_eval = Get_error_NN_log(alpha_default,L_default,M_default,x_train_0D,y_train)
Default_eval_Relu = Get_error_NN_Relu(alpha_default,L_default,M_default,x_train_0D,y_train)
