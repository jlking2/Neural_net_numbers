...
x_length_train = len(x_train)
x_length_test = len(x_test)
...
#p = np.shape(x_train_1D)[1]-1 
#n = np.shape(x_train_1D)[0]
...
M = np.shape(x_train_1D)[1]
p1 = 6
Q = 1

W = np.random.randn(p1,M)
V = np.random.randn(Q,p1)

alpha = 0.1 #step size
L = 10 #number of epochs


x_select = x_train_1D[6,:]
z_layer1 = x_select.T@W.T
H = 1/(1+np.exp(-z_layer1))
z_layer2 = H.T@V.T
Yhat = 1/(1+np.exp(-z_layer2))

#Backpropagate
delta = (Yhat-y_train[6])*Yhat*(1-Yhat)
Vnew = V-alpha*delta*H.T
gamma = np.zeros((p1,1))
for ii in range(0,p1):
    gamma[ii] = delta@V[0,ii]@H[ii]@(1-H[ii])
    Wnew = W - alpha*x_train_1D[[6],:].T@gamma

#gamma = delta@V.T*H*(1-H[:,1:])
#Wnew = W - alpha*x_train_1D[[6],:].T@gamma
V = Vnew
W = Wnew
...
for epoch in range(L):
    ind = np.random.permutation(x_length_train)
    for i in ind:
        # Forward-propagate
        x_select = x_train_1D[i,:]
#        w_select = 
        z = x_select.T@W
#        sigma = 1/
        
        H = logsig(np.hstack((np.ones((1,1)), x_train_1D[[i],:]@W)))
        Yhat = logsig(H@V)
        # Backpropagate
        delta = (Yhat-y_train[[i],:])*Yhat*(1-Yhat)
        Vnew = V-alpha*H.T@delta
        gamma = delta@V[1:,:].T*H[:,1:]*(1-H[:,1:])
        Wnew = W - alpha*x_train_1D[[i],:].T@gamma
        V = Vnew
        W = Wnew
    print(epoch)

## Final predicted labels (on training data)
H = logsig(np.hstack((np.ones((x_length_train,1)), x_train_1D@W)))
Yhat = logsig(H@V)
