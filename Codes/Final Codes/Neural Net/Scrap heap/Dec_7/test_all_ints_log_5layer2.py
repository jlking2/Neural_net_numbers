...
alpha_default = .2
L_default = 20 #60 is good 
p_default = 20 #80 is good
...
for epoch in range(L_default):
    ...
    for i in ind:
        ...
        V2new = V2 - alpha_default*delta2@G3.T
        
        delta3 = np.multiply(V2.T,np.multiply(G3,(1-G3)))@delta2
        V3new = V3 - alpha_default*delta3@G1.T
        
        gamma = np.multiply(V3.T,np.multiply(H,(1-H)))@delta3
        Wnew = W - alpha_default*gamma@x_train_1D[[i],:]
...
H_big = logsig(W@x_train_1D.T)
G3_big = logsig(V3@H_big)
G2_big = logsig(V2@G3_big)
