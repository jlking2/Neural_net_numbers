...
def eval_lasso( X, y, lambda_1 ):
    ...
    for j in range(max_iter):
        z = w - tau*(X.T@(X@w-y))
        w_old = w
        w = np.sign(z) * np.clip(np.abs(z)-tau*lambda_1/2, 0, np.inf)
...
def get_lasso_loop(x_train_mat, y_train, la_array):
    ...
    la_array = lambda_range

    #w_big contains [elements, numbers]        
    ...
    for rr in range(0,10):
        ...
        w_testo = get_lasso(x_train_0D, y_eval, la_array)
        w_giant[:,:,rr] = w_testo
...
def get_w_lasso(x_train_mat, y_train, lambda_1):
    x_width = np.shape(x_train_mat)[1]
    y_length = len(y_train)
    ...
    for rr in range(0,10):
        ...
        for k in range(0,y_length):
            if y_train[k] == number_eval:
                ...
                y_eval[k] = -1    
        w_big[:,rr] = eval_lasso(x_train_mat, y_eval, lambda_1)
    return w_big
...
lambda_1 = 1E-5
#w_big = get_w_lasso(x_train_0D, y_train, lambda_1)
#Err_test = get_error(x_test_0D, y_test, w_big)
...
w_giant = get_lasso_loop(x_train_0D,y_train,lambda_range)
...
Err_test = get_error(x_test_0D, y_test, w_biggo)



#Assemble W_big for each wavelength




    
#Err_test = get_error(x_test_0D, y_eval, w_big)
