...
for epoch in range(L_default):
    ...
    for i in ind:
        # Forward-propagate
        x_slice = x_train_1D[i,:]
        for kk in range(0,p):
            z = W[kk,:]@x_slice
            h[kk] = np.maximum(z,0)
        H = np.maximum(W@(x_train_1D[[i],:]).T,0)
        
        h_slice = np.squeeze(h)
        for kk in range(0,q):
            z = V[kk,:]@h_slice
            y_hat[kk] = np.maximum(z,0)
        Yhat = np.maximum(V@h,0)
        
    
        #H = np.maximum(np.hstack((x_train[[i],:]@W,np.ones((1,1)))),0)
        #print(np.shape(H))
        #print('IT IS')
        #time.sleep(5)
...
#        Wnew = W - alpha_default*x_train_1D[[i],:].T@gamma
#        V = Vnew
#        W = Wnew
#        time.sleep(4)
...
H_big = np.maximum(W@x_train_1D.T,0)
Yhat_big = (np.maximum(V@H_big,0)).T
